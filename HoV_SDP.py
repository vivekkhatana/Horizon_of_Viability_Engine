# -*- coding: utf-8 -*-
"""
Created on Thurs Oct 28 13:11:05 2021 
Centralized HoV dispatch code based on SDP RELAXATION of the HoV Integrer Program.
...
### LOG FILE LOCATIONS ###
SCENARIO_LOG_FILENAME = "hovLog_SDP.csv" #log file where
...
def HoVengine(Vec,Horizon):
   ...
   # 1. CHANGE THE IMPLEMENTATION OF THE HOV ENGINE USING SDP REALXATIONS AND NEW CONSTRAINTS
   # 2. CODE THE POWER APPORTIONING SCRIPT
   # 3. WRTIE CODE TO DECIDE RHO_d FOR THE APPORTIONING LAYER BASED ON CURRENT SOC
   # 4. STEP 2 WILL REQUIRE YOU TO FIGURE OUT WHICH CODE/TERMINAL GETS THE FLAG OF POWER APPROTIONING COMPLETED
   # 5. STEP 3 WILL REQUIRE YOU TO FIGURE OUT SHOULD THIS BE A COST OR A SEPARATE SUB-ROUTINE
    ...
    SOCmin = 0; SOEmin = 0.05
    discharg_thres = 0.70; charg_thres = 0.30;
    ...
    # Battery energy
    nontype4 = Nd+Nc+Npv
    Eb_initial = w_kw*ConfigData[nontype4:nontype4 + Nb,7] #np.array([120000,120000]) 
    
    # Diesel Genset energy
    Eg_initial = w_kw*ConfigData[nontype4 + Nb:nontype4 + Nb + Ng,7] #np.array([70000,70000])
    ...
    if HoV_ON == 1:   
        if np.max(SOC) > SOCmin or np.max(SOE) > SOEmin:
            ...
            Pslk = cvx.Variable(T)
            beta = cvx.Variable(1)
            X = cvx.Variable((Nd*T,Nd*T), symmetric = True)
            Y = cvx.Variable((Nd*T,Nd*T), symmetric = True)
            Z = cvx.Variable((Nd*T,Nd*T), symmetric = True)
            ...
            xt = cvx.diag(X)
            xt = cvx.reshape(xt,(Nd,T))            
            
            dummy = np.ones((Nd,T))-xt
            ...
            term01 = np.zeros(Nbactive)
            term02 = np.zeros(Nbactive)
            for i in range(Nbactive):
                ...
                term01[i] = np.max([0,SOC[Activebatt[i]] - discharg_thres])
                term02[i] = np.max([0,charg_thres - SOC[Activebatt[i]]])
            ...
            Cost = -np.ones(T)@C1 + np.ones(T)@Cload + np.ones(T)@C2 + beta
            ...
            dummy2 = cvx.multiply(0.5*(1 + xt),PLi)
            ...
            Mx1 = cvx.hstack([Y,X])
            Mx2 = cvx.hstack([X.T,Z])
            Mx = cvx.vstack([Mx1,Mx2])
            ...
            Constraints.extend([
                                 ...
                                 # -Pbmax0*np.ones(T)*np.max([np.max([0,charg_thres - SOC0])/(charg_thres - SOC0), 0]) <= Pb[0,:], Pb[0,:] <= np.max([np.max([0,SOC0 - discharg_thres])/(SOC0 - discharg_thres), 0])*Pbmax0*np.ones(T),\
                                 # -Pbmax1*np.ones(T)*np.max([np.max([0,charg_thres - SOC1])/(charg_thres - SOC1), 0]) <= Pb[1,:], Pb[1,:] <= np.max([np.max([0,SOC0 - discharg_thres])/(SOC0 - discharg_thres), 0])*Pbmax1*np.ones(T),\
                                 # -Pbmax2*np.ones(T)*np.max([np.max([0,charg_thres - SOC2])/(charg_thres - SOC2), 0]) <= Pb[2,:], Pb[2,:] <= np.max([np.max([0,SOC0 - discharg_thres])/(SOC0 - discharg_thres), 0])*Pbmax2*np.ones(T),\
                                 # -Pbmax3*np.ones(T)*np.max([np.max([0,charg_thres - SOC3])/(charg_thres - SOC3), 0]) <= Pb[3,:], Pb[3,:] <= np.max([np.max([0,SOC0 - discharg_thres])/(SOC0 - discharg_thres), 0])*Pbmax3*np.ones(T),\
                                 # -Pbmax4*np.ones(T)*np.max([np.max([0,charg_thres - SOC4])/(charg_thres - SOC4), 0]) <= Pb[4,:], Pb[4,:] <= np.max([np.max([0,SOC0 - discharg_thres])/(SOC0 - discharg_thres), 0])*Pbmax4*np.ones(T),\
                                 # -Pbmax5*np.ones(T)*np.max([np.max([0,charg_thres - SOC5])/(charg_thres - SOC5), 0]) <= Pb[5,:], Pb[5,:] <= np.max([np.max([0,SOC0 - discharg_thres])/(SOC0 - discharg_thres), 0])*Pbmax5*np.ones(T),\
                                 # -Pbmax6*np.ones(T)*np.max([np.max([0,charg_thres - SOC6])/(charg_thres - SOC6), 0]) <= Pb[6,:], Pb[6,:] <= np.max([np.max([0,SOC0 - discharg_thres])/(SOC0 - discharg_thres), 0])*Pbmax6*np.ones(T),\
                                 ...
                                 X >> 0,\
                                 Mx >> 0,\
                                 cvx.trace(Y) + cvx.trace(Z) <= 2*beta
            ...
            prob.solve(solver = cvx.MOSEK, verbose=False, warm_start=True)
            ...
            if prob.status == 'optimal':
                ...
                if endTime < T:
                    ...
                    Pbsetpt = np.zeros(Nb)
                    Pbsetpt[Activebatt[:]] = Pbmax[Activebatt[:]]
                    ...
                    PGenWithoutSlack = np.sum(PredPVOutput[T-1,:]) + np.sum(Pbsetpt) + np.sum(Pgsetpt)                    
                    Pslksetpt = PloadON -  PGenWithoutSlack 
...
def receiveMeasHoV():
    ...
    while threadGo:
        ...
        if len(vector) == inputmeasveclen:   
            ...
            lmMeasVec = np.concatenate((dt,vector),axis=None)
            
            #if enabled, immediately relay this vector on to the NLM engine
            if lmMeasRelay:
                p = len(lmMeasVec) #determine how many floats we are sending\n",
                msgBytes = struct.pack('<{}'.format('f'*p),*lmMeasVec.tolist()) #bytepack numpy row vector - use little Endian format (required by Opal-RT)\n",
                s.sendto(msgBytes, lmMeasIP)
                log.debug("Relayed measurement packet to Load Management engine on " + str(lmMeasIP))
                # log.debug(nlmVec)               
...
if __name__ == "__main__":
    ...
    ap.add_argument("--lmMeasRelayPort", help="port on which the measurements received by HoV are realyed to the Load Management engine", default="5000")
    args = ap.parse_args()
    ...
    lmMeasRelayPort = int(args.lmMeasRelayPort)
    if lmMeasRelayPort != -1:
        #check that UDP port specified is within range
        if lmMeasRelayPort > 100 and lmMeasRelayPort < 100000:
            lmMeasRelay = True
            lmMeasIP = (args.localip, lmMeasRelayPort)
        else:
            log.error("ERROR: LM Relay port must be between 100-100000")
            sys.exit()
