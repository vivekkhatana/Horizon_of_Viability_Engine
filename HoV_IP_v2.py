...
Implements a charging region for the battery. One the battery hits th charging threshold it should be charged until 
...
def HoVengine(Vec,Horizon):
    ...
    discharg_thres = 0.60; charg_thres = 0.20; 
    ...
    if HoV_ON == 1:   
        if np.max(SOC) > SOCmin or np.max(SOE) > SOEmin:
            ...
            mosek_params = {
                ...
                "MSK_DPAR_MIO_MAX_TIME": 120,
                "MSK_DPAR_OPTIMIZER_MAX_TIME": 120
            }
                
            prob = cvx.Problem(cvx.Minimize(Cost), Constraints)
        
            prob.solve(solver = cvx.MOSEK, mosek_params=mosek_params, verbose=False)
