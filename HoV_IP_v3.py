# -*- coding: utf-8 -*-
"""
Created on Sat Feb  5 06:16:33 2022
...
Battery operation has 3 regions: 1. Always charge, 2. Always discharge, 3.Charge/discharge based on future load and generation.
...
def HoVengine(Vec,Horizon):
    ...
    discharg_thres = 0.8; # always discharge for SOC >= discharg_thres
    charg_thres = 0.15; # always charge for SOC <= charg_thres    
    chargingUp = 0.40; # if SOC <= charg_thres, then charge upto SOC = chargingUp   
    ...
    for i in range(Nb):
        if (SOC[i] < charg_thres):
            IsChargingReqd[i] = 1
        elif (SOC[i] > charg_thres) and (SOC[i] < chargingUp) and (IsChargingReqd[i] == 0):
            IsChargingReqd[i] = 0
        elif (SOC[i] > charg_thres) and (SOC[i] < chargingUp) and (IsChargingReqd[i] == 1):
            IsChargingReqd[i] = 1 
        elif (SOC[i] > chargingUp):  
            IsChargingReqd[i] = 0
                
    for i in range(Nb):
        if (SOC[i] >= discharg_thres):
            IsDisChargingReqd[i] = 1
        elif (SOC[i] < discharg_thres):
            IsDisChargingReqd[i] = 0            
    ...
    # controllable loads that are currently ON
    ...
    # warm start solution for the IP solver    
    ...
    if HoV_ON == 1:   
        if np.max(SOC) > SOCmin and np.max(SOE) > SOEmin:
            ...
            # dummy vector with each entry equal to the max capacity            
            ...
            # cost of battery usage depends on the operating region of the battery           
            term01 = IsDisChargingReqd # for only discharging part of the cost
            term02 = IsChargingReqd # for only charging part of the cost
            term03 = 1 - (IsChargingReqd + IsDisChargingReqd) # for normal operation part of the cost

            # discharging part of the cost
            term1 = np.multiply(term01, Cd)            
            term2 = dummyPbmax - Pb   
            discharge_cost = cvx.multiply(term1.reshape(Nbactive,1), term2)
            
            # charging part of the cost
            term3 = np.multiply(term02, Cc)            
            term4 = Pb
            charge_cost = cvx.multiply(term3.reshape(Nbactive,1), term4) 
             
            # normal operation part of the cost
            
            # cost of discharging the battery based on SOC
            discharge_indicator =  np.zeros(Nbactive)
            charge_indicator =  np.zeros(Nbactive)   
            scaling = 1

            for i in range(Nbactive):
                discharge_indicator[i] = SOC[Activebatt[i]] - discharg_thres
                charge_indicator[i] = scaling*(charg_thres - SOC[Activebatt[i]])
             
            term5 = np.multiply(term03, discharge_indicator)  
            term6 = np.multiply(term03, charge_indicator)

            normal_cost = cvx.multiply(term5.reshape(Nbactive,1), term2) + cvx.multiply(term6.reshape(Nbactive,1), term4)
            
            
            CBatt = discharge_cost + charge_cost + normal_cost
            ...
            consTerm2 = np.multiply(IsDisChargingReqd.reshape(Nbactive,1),dummyPbmax)
            consTerm3 = np.multiply((1-(IsChargingReqd+IsDisChargingReqd)).reshape(Nbactive,1),dummyPbmax)            
            
            BattMaxCap = consTerm2 + consTerm3
            BattMinCap = -(consTerm1 + consTerm3)
            ...
            Constraints.extend([ 
                                 ...
                                 BattMinCap.T.flatten() <= cvx.vec(Pb), cvx.vec(Pb) <= BattMaxCap.T.flatten(),\
