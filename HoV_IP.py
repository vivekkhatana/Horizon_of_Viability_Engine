...
### LOG FILE LOCATIONS ###
SCENARIO_LOG_FILENAME = "PowerSystemState_Hov.csv" #log file where
...
def HoVengine(Vec,Horizon):
    ...
    # Battery energy
    nontype4 = Nd+Nc+Npv
    Eb_initial = ConfigData[nontype4:nontype4 + Nb,7] #np.array([120000,120000]) 
    
    # Diesel Genset energy
    Eg_initial = ConfigData[nontype4 + Nb:nontype4 + Nb + Ng,7] #np.array([70000,70000])
    
    # Deferrable loads cost vector    
    CL = ConfigData[0:Nd,3]
    Plmax = ConfigData[0:Nd,6]
    ...
    # Ebd0 = SOC0*Eb_initial[0]*dt; Ebd1 = SOC1*Eb_initial[1]*dt; Ebd2 = SOC2*Eb_initial[2]*dt; 
    # Ebd3 = SOC3*Eb_initial[3]*dt; Ebd4 = SOC4*Eb_initial[4]*dt; Ebd5 = SOC5*Eb_initial[5]*dt;
    # Ebd6 = SOC6*Eb_initial[6]*dt;    
    SOCPrime = 1 - SOC
    Ebc = np.multiply(SOCPrime,Eb_initial)*time_correction_hrs_to_sec
    # Ebc0 = (1-SOC0)*Eb_initial[0]*dt; Ebc1 = (1-SOC1)*Eb_initial[1]*dt; Ebc2 = (1-SOC2)*Eb_initial[2]*dt; 
    # Ebc3 = (1-SOC3)*Eb_initial[3]*dt; Ebc4 = (1-SOC4)*Eb_initial[4]*dt; Ebc5 = (1-SOC5)*Eb_initial[5]*dt;
    # Ebc6 = (1-SOC6)*Eb_initial[6]*dt;   
    ...
    Pc = loadData[InitTime:InitTime + T, Nd + Nc]
    
    # log.info(Pc)
    
    # Controllable/deferable loads instantaneous power demand
    PLi = loadData[InitTime:InitTime + T, 1:Nd + Nc].T
    # Nd,Nimp = PLi.shape
    
    # available PV power
    PV = np.zeros(Npv)
    for i in range(Npv):
        PV[i] = -Vec[Nd + Nc + 1 + 2*i]
    ...
    for i in range(Nb):
        Pbmax[i] = -Vec[nontype4PowerMeas + 1 + (3*i+1)]
    ...
    Pgmax = np.zeros(Ng)
    for i in range(Ng):
        Pgmax[i] = -Vec[nontype4PowerMeas + Nb*3 + 1 + (3*i+1)]
    ...
    Pslkmax = -Vec[nontype4PowerMeas + Nb*3 + Ng*3 + 1 + 1]
    Pslkmin = -Vec[nontype4PowerMeas + Nb*3 + Ng*3 + 1 + 2]
    ...
    if HoV_ON == 1:   
        if np.max(SOC) > SOCmin or np.max(SOE) > SOEmin:
            ...
            # dummyPbmax = np.ones((Nbactive,T))
            # dummyPgmax = np.ones((Ngactive,T))
            # for tt in range(T):
            #     dummyPbmax[:,tt] = Pbmax[Activebatt[:]]
            #     dummyPgmax[:,tt] = Pgmax[Activediesel[:]]
            ...
            # for i in range(Nbactive):
            #     term2[i,:] = term1[i]*(dummyPbmax[i,:] - Pb[i,:])

            # term2 = dummyPbmax - Pb

            term3 = np.multiply(term02, Cc)
            
            # term4 = cvx.zeros((Nbactive,T)) 
            # for i in range(Nbactive):
            #     term4[i,:] = term3[i]*Pb[i,:]
            ...
            # log.info(np.ones(Ngactive)@Pg + np.ones(Nbactive)@Pb)
            # log.info(PredPVOutput@np.ones(Npv))
                            
            # consTerm1 = np.max([0,charg_thres - SOC[Activebatt[:]]])
            consTerm2 = charg_thres - SOC[Activebatt[:]]
            consTerm2 = np.reciprocal(consTerm2)
            consTerm3 = np.multiply(term02,consTerm2)
            
            
            consTerm4 = np.zeros(Nbactive)
            for i in range(Nbactive):
                consTerm4[i] = np.max([consTerm3[i],0])
            
            # consTerm4 = np.max([consTerm3[i],0])
            # consTerm5 = np.ones((Nbactive,T))
            # for i in range(T):
            #     consTerm5[:,i] = np.multiply(Pbmax[Activebatt[:]],consTerm4)
            
            consTerm5 = np.multiply(consTerm4.reshape(Nbactive,1), dummyPbmax)
            ...
            Constraints.extend([ 
                                 ...
                                 -consTerm5.T.flatten() <= cvx.vec(Pb), cvx.vec(Pb) <= dummyPbmax.T.flatten(),\
                                 ...
                                 0.99*Pslkmin*np.ones(T) <= Pslk, Pslk <= 0.99*Pslkmax*np.ones(T),\
            ...
            mosek_params = {
                ...
                "MSK_IPAR_INTPNT_MAX_ITERATIONS": 5
            }
                
            prob = cvx.Problem(cvx.Minimize(Cost), Constraints)
        
            prob.solve(solver = cvx.MOSEK, mosek_params=mosek_params, verbose=True)
            ...
            if prob.status == 'optimal':
                ...
                if endTime <= T:
                    ...
                    for i in range(Nbactive):
                        if SOC[Activebatt[i]] > discharg_thres:
                    ...
                    PloadON = Pc[endTime] + np.sum(PLi[endTime,ONindex])  # total load power ON
                    ...
                    PGenTotal = np.sum(PredPVOutput[T-1,:]) + np.sum(Pbsetpt) + Pgsetpt + Pslksetpt
                    ...
                    PloadON = Pc[T-1] + np.sum(PLi[T-1,ONindex])  # total load power ON
        ...
        if measurement_reception_started == 1:
            ...
            if HoV_ON == 1:
                ...
                if opt_sol_found == 1:                
                    ...
                    HoVcmd = np.concatenate((LoadStatus, Pbsetpt, Pgsetpt), axis=None)  
                    sendOutHov(HoVcmd)
                    # time.sleep(60)
                else:
                    t2 = time.time() - tStart
